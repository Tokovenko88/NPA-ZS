...
- **DO NOT** propose changing punctuation at the end of sentences (period vs semicolon vs comma) unless the amending instruction explicitly mandates such a change. If a sentence ends with a period where it previously ended with a semicolon (or vice versa), and this is NOT described in the instructions, treat it as an intentional stylistic choice and DO NOT flag it as an error.
...
- **EXCEPTION to the previous rule:** every norm listed in `<coverage_gaps>` (if this section is present in the prompt) MUST be reported as an issue with `kind: "not_applied"`, even though it has no entry in `<changes>`. `<coverage_gaps>` is computed deterministically from the change tracker (a norm marked applied but with no revision created by the amending law in the result) and CANNOT be dismissed or explained away.
- **COVERAGE GAPS ARE ADDITIONAL WORK, NOT A REPLACEMENT:** verifying `<coverage_gaps>` never replaces the mandatory full verification of EVERY entry in `<changes>`. After handling the gaps, you must still analyze every single `<changes>` entry against `<instructions>` and report its real errors (lost mandatory phrases, wrong place, truncation). An answer that only reports `<coverage_gaps>` and skips the `<changes>` verification is a FAILED verification.
- **NEVER report an issue that only acknowledges correctness.** If the application of an instruction is correct — do NOT include it in `issues`, do not write "изменение применено корректно" as an issue, and do not set the verdict to `incorrect` because of a correct application. Every reported issue must describe a REAL semantic defect and provide a `corrections` entry where possible.
- **For `kind=repel_law`:** the element being marked `not_valid` (утратившим силу) IS the correct and complete application of the instruction — no new revision is created for it by design. Never flag a `repel_law` entry as an error merely because no new revision was created, and never propose creating a text revision for a repelled element.
...
- Work through ALL entries in `<changes>` one by one before producing the verdict; the most dangerous failure mode is missing a real text-loss error (e.g. a mandatory phrase lost in the middle of a sentence) because attention was consumed by `<coverage_gaps>`.