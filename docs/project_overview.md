...
```
## Сравнение редакций НПА (модуль `compare`)

Модуль `src/compare/` сравнивает RTF-документ, сформированный проектом, с документом
того же НПА, сформированным сторонней правовой системой (DOCX/DOC/RTF/HTML/TXT),
и формирует отчёт в Markdown.

### Запуск

```bash
python scripts/run_compare.py                  # GUI (2 поля файлов + журнал)
python scripts/run_compare.py --ours ours.rtf --theirs theirs.docx --mode mechanical
make run-compare
```

CLI-эквивалент: `npazs compare --ours ... --theirs ... [--output report.md]
[--mode mechanical] [--backend ...] [--model ...]`.

### Конвейер

1. `converters.py` — извлечение текста и блоков: RTF (собственный парсер
   управляющих слов, cp1251), DOCX (zip + XML), DOC (эвристика), HTML, TXT.
2. `normalizer.py` — нормализация текста, извлечение примечаний
   (`parse_note`, `extract_notes`) с датами вступления в силу и номерами
   изменяющих НПА; блоки примечаний отделяются от тела независимо от позиции.
3. `tree.py` — разбор структурных заголовков (раздел/глава/статья/часть/пункт)
   в элементы с каноническим ключом пути и человекочитаемым путём.
4. `differ.py` — посимвольное сравнение одноимённых элементов
   (`difflib.SequenceMatcher` на символах — важна каждая буква и знак
   препинания) и сопоставление добавленных/удалённых элементов.
5. `agent_compare.py` — классификация причин расхождений: LLM-агент
   (промпт `data/prompts/compare_prompt.md`; причины: original_edition,
   amendment, implementation_gap, technical_correction, formatting, unclear)
   либо механический фолбэк по примечаниям.
6. `npa_resolver.py` — подтягивание из базы JSON: текст конкретного изменения
   (по `source_item_id` ссылки «через #» либо по пути элемента), полный текст
   изменяющего НПА (если элемент не определён), текст того же элемента из
   исходной редакции (`get_original_element_text`) для проверки
   «технических правок» правовой системы.
7. `report_builder.py` — отчёт: раздел «Примечания» (наличие + даты вступления
   в силу; стилистика оформления не сравнивается) и раздел «Текст НПА»
   (посимвольные расхождения с полным путём к структурному элементу,
   причиной и цитатой изменения из базы).

### Возобновление после сбоя

Состояние сохраняется в `<отчёт>.checkpoint.json` после каждого пакета
классификации. Повторный запуск с теми же файлами (отпечаток: пути + размеры +
mtime + режим) продолжает с первого необработанного различия — сравнение не
начинается заново. Отчёты: `data/output/compare/*.md`; журналы:
`data/logs/compare_*.log`.
